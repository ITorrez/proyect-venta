...
class Compra_Producto extends Conexion{
	...
	{
		$sql="SELECT (cp2.id_compra_producto)as idcompraprod, 
		             ...
		             (cp2.stock_actual)as stokactual,
		             (cp2.precio_venta_prod)as precioVentaprod,
		             (cp2.precio_venta_prod_Fact)as precioVentaprodFact 
		        FROM tb_compra_producto as cp2
		  INNER JOIN tb_producto
		          ON tb_producto.id_producto=cp2.id_producto 
		  INNER JOIN tb_marca
		          ON tb_marca.id_marca=tb_producto.id_marca
		  INNER JOIN tb_categoria 
		          ON tb_categoria.id_categoria=tb_producto.id_categoria
		   INNER JOIN (SELECT cp.id_producto,MAX(cp.precio_venta_prod) as precio_maximo 
                       FROM tb_compra_producto as cp WHERE cp.stock_actual>0 
                       GROUP BY cp.id_producto) AS maximos 
                ON cp2.id_producto=maximos.id_producto 
               AND cp2.precio_venta_prod=maximos.precio_maximo
		       WHERE cp2.stock_actual>0 
		         and cp2.estado='Activo' 
		       GROUP BY cp2.id_producto
		       ";
...
WHERE tb_producto.id_producto=tb_compra_producto.id_producto AND tb_compra_producto.id_compra_producto=$idcompra";
   return parent::ejecutar($sql);
	...
	INNER JOIN tb_compra_producto as b 
	       ON  a.id_producto=b.id_producto 
   INNER JOIN tb_marca as c 
           ON c.id_marca= a.id_marca
	INNER JOIN tb_categoria as d 
	       ON a.id_categoria=d.id_categoria
			WHERE b.stock_actual>0
			   and b.estado='Activo'
	...
	public function mostrarUnaCompraProductoDeCompra($idcompra)
	{
		$sql="SELECT    b.cantidad_compra,
				          b.precio_unit_compra,
				          b.precio_venta_prod,
				          b.precio_venta_prod_Fact,
				          b.precio_tope,
	...
	public function darBajaStockCompraProducto($idcompra)
	{
     $sql="UPDATE tb_compra_producto
              SET cantidad_compra='$this->cantidad_compra',
                  stock_actual='$this->stock_actual',
                  subtotal_compra='$this->subtotal_compra'
            WHERE id_compra=$idcompra ";
      if (parent::ejecutar($sql)) 
			 return true;
		 else
