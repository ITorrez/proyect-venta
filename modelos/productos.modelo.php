...
WHERE tb_marca.id_marca=tb_producto.id_marca AND tb_categoria.id_categoria=tb_producto.id_categoria AND estado_producto='Activo' AND (stok_facturado >0 OR stock_simple >0 )";
	...
	public function verificaNombProd($nomProd)
	{
		$sql="SELECT id_producto 
		        FROM tb_producto
			   WHERE nombre_producto='$nomProd'";
			return parent::ejecutar($sql);
	}
	public function verificaCodProd($CodProd)
	{
		$sql="SELECT id_producto 
		        FROM tb_producto
	...
	public function verificaNombProd_exeptoID($nomProd,$idprod)
	{
		$sql="SELECT id_producto 
		        FROM tb_producto
			   WHERE nombre_producto='$nomProd'
	...
	public function verificaCodProd_exeptoID($CodProd,$idprod)
	{
		$sql="SELECT id_producto 
		        FROM tb_producto
			   WHERE codigo_producto='$CodProd'
