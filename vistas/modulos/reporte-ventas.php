...
<div id="modalElimventa" class="modal fade" role="dialog">
  ...
  <!-- Modal -->
<div id="modalModFechaventa" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-header" style="background: #f39c12; color:white;">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
        <h4 class="modal-title">Actualizar Fecha venta</h4>
      </div>
      <div class="modal-body">
         <div class="box-body">

            <div class="form-group">
              <label>Còdigo de venta</label>
              <div class="input-group">
                  <span class="input-group-addon"><i class="fa fa-tag"></i></span>
                  <input type="text" class="form-control input-lx" name="id_venta" id="id_venta" placeholder="id venta" required="" readonly="">
                 
              </div>  
            </div>

            <div class="form-group">
              <label>Fecha actual de venta</label>
              <div class="input-group">
                  <span class="input-group-addon"><i class="fa fa-calendar"></i></span>
                 <input type="text" class="form-control input-lx" name="fecha_venta_actual" id="fecha_venta_actual" placeholder="Ingresar fecha" required="" readonly="">
              </div>  
            </div>

            <div class="form-group">
              <label>Fecha nueva día/mes/año</label>
              <div class="input-group">
                  <span class="input-group-addon"><i class="fa fa-user"></i></span>
                 <input type="date" class="form-control input-lx" name="fecha_nueva_venta" id="fecha_nueva_venta" value="<?php echo $fecha; ?>" required="">
              </div>  
            </div>
      ...
      <div class="modal-footer">
        ...
        <button type="submit" class="btn btn-primary" name="btnactfecha" id="btnactfecha">Actualizar</button>
      </div>
...
/*===================FUNCION QUE LLAMA AL MODIFICAR FECHA VENTA ===========================================*/
$(document).ready(function() { 
   $("#btnactfecha").on('click', function() {
  
   var formDataelim = new FormData(); 
   
   var btnactfecha=$('#btnactfecha').val();
   var id_venta    =$('#id_venta').val();
   var fecha_nueva_venta=$('#fecha_nueva_venta').val();
   
 
   if ( (id_venta=='') || (fecha_nueva_venta=='')  ) 
   {
     setTimeout(function(){  }, 2000); swal('ATENCION','Deve de completar todos los campos','warning'); 
   ...
   else
   {
     ...
     formDataelim.append('id_venta',id_venta);
     formDataelim.append('fecha_nueva_venta',fecha_nueva_venta);
     formDataelim.append('btnactfecha',btnactfecha);
      $.ajax({ url: 'controladores/venta.controlador.php', 
               type: 'post', 
               data: formDataelim, 
               contentType: false, 
               ...
               success: function(response) { 
                console.info(response);
                //    var posOk=response[1];
                    var posIdpregunta=response[1];
                    console.info(posIdpregunta);
                  if (response==0) 
                  {
                    
                    setTimeout(function(){ location.href='reporte-ventas'; }, 2000); swal('EXELENTE','Actualizado correctamente','success'); 
                  ...
                  else
                  {
                    if (response==-1) 
                    {
                    setTimeout(function(){  }, 2000); swal('ERROR','Intente nuevamente, no se actualizo la fecha de venta, codigo de error '+response,'error');
                    }
                    else
                    {
                      if (response>0) 
                      {
                        setTimeout(function(){  }, 2000); swal('ERROR La venta esta en un cierre de caja, debe eliminar el cierre de caja con codigo'+response+' para poder actualizar la fecha de venta',' ','error');
                      }
                    }
