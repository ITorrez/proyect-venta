                 ...
                 <select class="form-control select2 input-lx" name="slectcli" id="slectcli">
                   ...
                   <?php
                    ...
                    while ($filcli=mysqli_fetch_object($result)) 
                     {
                      ?> 
                    <option value="<?php echo $filcli->id_cliente; ?>"><?php echo $filcli->id_cliente.' '.$filcli->nombre_cliente.' '.$filcli->apellido_cliente; ?></option>
                      <?php
                     }
                   ?>
...
<script type="text/javascript">
  var tipousuario= $("#texttipouser").val();
  var readonlytext=''
...
// CARGAMOS EL ID DEL PRODUCTO AL ARRAYDE PRODUCTOS SELECCIONADOS
  ...
  '<!--producto-->'+
     ...
     '<td> <input type="text" id="txtprecioProdofi'+id+'" name="txtprecioProdofi'+id+'" value="'+precio_venta+'" readonly></td>'+
     '<td> <input type="text" oninput="calculosubtotalprecioventa(this.value,'+id+')" id="txtprecioProd'+id+'" name="txtprecioProd'+id+'" value="'+precio_venta+'" '+readonlytext+'></td>'+
     ...
     '<td> <input type="text" class="nuevoPrecio" name="preciovendido" id="precVenta'+id+'" oninput="sumarTotalPrecio()" precioReal="'+precio_venta+'" value="'+precio_venta+'" readonly="" > </td>'+
...
<script type="text/javascript">
  function calculosubtotal(cantidad,idprod,precVenta,stockTotal)
  {
   ...
   var idtextsubtotal="precVenta"+idprod;/*obtenemos el id del input subtotal*/
    var txtprecioventa="txtprecioProd"+idprod;
    var precioVentatext=$("#"+txtprecioventa+"").val()
   // alert(precioVentatext)
   var subtotalprod=cantidad*precioVentatext;
  ...
  function calculosubtotalprecioventa(precioVentatext,idprod)
  {
    var textcantidad="txtcantidad"+idprod;
    var cantidadProd=$("#"+textcantidad+"").val()
    var subtotalprod=cantidadProd*precioVentatext;
    var idtextsubtotal="precVenta"+idprod;/*obtenemos el id del input subtotal*/
    $("#"+idtextsubtotal+"").val(subtotalprod.toFixed(2));
     sumarTotalPrecio();/*FUNCION QUE SUMA EL MONTO TOTAL DE LA VENTA*/
  }
    ...
    while(contadoridP<totaldeprodventa)
        {
              ...
              var idtextprecioprod="txtprecioProdofi"+arrayIdproductosVenta[contadoridP];/*obtenemos el id del txt precio del producto venta*/
            ...
            {
              var tipo_user=$("#texttipouser").val();
              if (tipo_user=='admin')//verificamos si es usuario administrador para no hacer la validacion ya que el admin puede vender a cualquier precio
              {
                return 0;
              }
              else{
