...
if (isset($_POST["btnguardarventa"])) 
{
 ...
 if (isset($_POST["btnactfecha"])) 
 {
 	ctrlModificarFechaVenta();
 }
...
/*-----------MODIFICACION DE FECHA DE VENTA-------------*/
 function ctrlModificarFechaVenta()
 {
 	 $idventa=$_POST['id_venta'];
 		$objventa=new Venta();
 		$idcierrecaja=0;
 		$resulveri=$objventa->verificarVentaEnCierre($idventa);//verificamos si la venta esta en un cierre
 		while($fcierre=mysqli_fetch_object($resulveri))
 		{
           $idcierrecaja=$fcierre->id_cierre_caja;
 		}
 		if ($idcierrecaja>0) //si ya esta en un cierre muestra mensaje
 		{
 			echo $idcierrecaja;//muestra el mensaje que dice que la venta esta en un cierre
 		}
 		else
 		{
 			$objventa->set_fechaventa($_POST['fecha_nueva_venta']);
 			if ($objventa->actualizarFechaVenta($idventa)) 
 			{
 				echo 0;//actualizado correctamente
 			}
 			else {
 				echo -1;//error en la actualizacion
 			}
